...
def replace_all_existing(text: str, pattern: str, replacement: str, label: str) -> str:
...
def update_flake_prebuilt(version: str, nix_hash: str) -> None:
    ...
    text = replace_once(
        text,
        r'url = "https://github\.com/MathieuMoalic/koun/releases/download/[^"]+";',
        f'url = "{url}";',
    ...
    if 'sourceRoot = ".";' in text:
        text = text.replace(
            '      sourceRoot = ".";\n', "      dontUnpack = true;\n", 1
        )
    elif "dontUnpack = true;" not in text:
        text = text.replace(
            "      installPhase = ''\n",
            "      dontUnpack = true;\n\n      installPhase = ''\n",
            1,
    ...
    text = re.sub(
        r"(?s)installPhase = ''\n\s*install -Dm755 .*? \$out/bin/koun\n\s*'';",
        "installPhase = ''\n        install -Dm755 $src $out/bin/koun\n      '';",
        text,
        count=1,
    )
...
def bump_command(bump_type: str) -> None:
    ...
    print(f"✓ Bumped version files to {new}")
